...
public:
    ...
    void insert_position(int position, int value){
      if(position == 0) return insert_start(value);
      if(position == (length-1)) return insert_end(value);
    ...
    void delete_position(int position){
      if(position == 0) return delete_start();
      if(position == (length-1)) return delete_end();
    ...
    void display() {
      cout<<"LinkedList length:: "<<length<<" "<<endl;
